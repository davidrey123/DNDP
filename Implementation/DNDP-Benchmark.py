...
def algo_SOIC(data):
    ...
    xt = {(i,j): SOIC.continuous_var() for (i,j) in A}
    x = {(i,j): SOIC.continuous_var() for (i,j) in A}
    xc = {(i,j,s): SOIC.continuous_var() for (i,j) in A for s in D}
    y = {(i,j): SOIC.binary_var() for (i,j) in A2}
    xy = {(i,j): SOIC.continuous_var() for (i,j) in A2}
    ...
    for (i,j) in A:
        SOIC.add_constraint(sum(xc[i,j,s] for s in D) == x[i,j])
        ...
        if (i,j) in A1:
            SOIC.add_constraint(xt[i,j] == T[i,j]*x[i,j]
                               + c[i,j]*sum(ll[i,j,v]*(a[i,j,v-1]**(e[i,j]+1)) + lr[i,j,v]*(a[i,j,v]**(e[i,j]+1)) for v in Vp))
        else:
            SOIC.add_constraint(xt[i,j] == T[i,j]*x[i,j]
                               + c[i,j]*sum(ll[i,j,v]*(a[i,j,v-1]**(e[i,j]+1)) + lr[i,j,v]*(a[i,j,v]**(e[i,j]+1)) for v in Vp)
                               + (x[i,j] - xy[i,j])*Mtt)
    ...
    for (i,j) in A2:
        SOIC.add_constraint(xy[i,j] <= x[i,j])
        SOIC.add_constraint(xy[i,j] >= x[i,j] - (1 - y[i,j])*Mflow)
        SOIC.add_constraint(xy[i,j] <= y[i,j]*Mflow)
        SOIC.add_constraint(xy[i,j] >= 0)
      
    for (i,j) in A:
        SOIC.add_constraint(xt[i,j] >= 0)
        SOIC.add_constraint(x[i,j] >= 0)
        for s in D:
    ...
    SOIC.minimize(sum(xt[i,j] for (i,j) in A))
    ...
    while converged == False:  
        ...
        xopt = {(i,j):SOIC.solution.get_value(x[i,j]) for (i,j) in A}
...
def model_SOBB(data,SP):
    ...
    xt = {(i,j): SOBB.continuous_var() for (i,j) in A}
    x = {(i,j): SOBB.continuous_var() for (i,j) in A}
    xc = {(i,j,s): SOBB.continuous_var() for (i,j) in A for s in D}
    y = {(i,j): SOBB.binary_var() for (i,j) in A2}
    xy = {(i,j): SOBB.continuous_var() for (i,j) in A2}
    ...
    for (i,j) in A:
        SOBB.add_constraint(sum(xc[i,j,s] for s in D) == x[i,j])
        ...
        if (i,j) in A1:
            SOBB.add_constraint(xt[i,j] == T[i,j]*x[i,j] 
                                   + c[i,j]*sum(ll[i,j,v]*(a[i,j,v-1]**(e[i,j]+1)) 
                                                + lr[i,j,v]*(a[i,j,v]**(e[i,j]+1)) for v in Vp))
        else:
            SOBB.add_constraint(xt[i,j] == T[i,j]*x[i,j] 
                                   + c[i,j]*sum(ll[i,j,v]*(a[i,j,v-1]**(e[i,j]+1)) 
                                                + lr[i,j,v]*(a[i,j,v]**(e[i,j]+1)) for v in Vp) + (x[i,j] - xy[i,j])*Mtt)

    for (i,j) in A2:
        SOBB.add_constraint(xy[i,j] <= x[i,j])
        SOBB.add_constraint(xy[i,j] >= x[i,j] - (1 - y[i,j])*Mflow)
        SOBB.add_constraint(xy[i,j] <= y[i,j]*Mflow)
        SOBB.add_constraint(xy[i,j] >= 0)
      
    for (i,j) in A:
        SOBB.add_constraint(xt[i,j] >= 0)
        SOBB.add_constraint(x[i,j] >= 0)
        for s in D:
    ...
    SOBB.minimize(sum(xt[i,j] for (i,j) in A))
    ...
    if SOBB.solve_details.status == 'integer infeasible':
        ...
        SP['xtopt'] = {(i,j):SOBB.solution.get_value(xt[i,j]) for (i,j) in A}
    return SP
...
output = open("Benchmark_budget",'w')
net = 'Siouxfalls'
gtime0 = time.time()
...
output = open("Benchmark_demand",'w')
net = 'Siouxfalls'
gtime0 = time.time()
